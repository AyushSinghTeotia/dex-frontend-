...
import { ChainId, JSBI, TokenAmount } from 'quest-cyswap-sdk'
...
import { MERKLE_ROOT, CYY } from './../../constants/index'
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
    ...
    const cyy = chainId ? CYY[chainId] : undefined
    ...
    if (!cyy) return undefined
    if (!canClaim || !userClaimData) {
        return new TokenAmount(cyy, JSBI.BigInt(0))
    }
    return new TokenAmount(cyy, JSBI.BigInt(userClaimData.amount))
}
            ...
            return distributorContract
                ...
                .then((response: TransactionResponse) => {
                    addTransaction(response, {
                        summary: `Claimed ${unClaimedAmount?.toSignificant(4)} CYY`,
