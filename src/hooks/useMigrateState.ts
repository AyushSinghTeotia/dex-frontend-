...
import useCyyRoll from './useCyyRoll'
...
const useMigrateState: () => MigrateState = () => {
    ...
    const { migrate, migrateWithPermit } = useCyyRoll(state?.selectedLPToken?.version)
    ...
    const onMigrate = useCallback(async () => {
        if (mode && state.selectedLPToken && account && library) {
            ...
            addTransaction(tx, {
                summary: `Migrate ${exchange} ${state.selectedLPToken.symbol} liquidity to CySwap`
            })
