...
import { ChainId, JSBI } from 'quest-cyswap-sdk'
...
import { useCyyRollContract } from 'hooks/useContract'
...
const MigrateButtons = ({ state, exchange }: { state: MigrateState; exchange: string | undefined }) => {
    ...
    const cyyRollContract = useCyyRollContract(
        state.selectedLPToken?.version ? state.selectedLPToken?.version : undefined
    )
    console.log(
        'cyyRollContract address',
        cyyRollContract?.address,
        state.selectedLPToken?.balance,
        state.selectedLPToken?.version
    )

    const [approval, approve] = useApproveCallback(state.selectedLPToken?.balance, cyyRollContract?.address)
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="text-sm text-low-emphesis text-center">
                {i18n._(
                    t`Your ${exchange} ${state.selectedLPToken.tokenA.symbol}/${state.selectedLPToken.tokenB.symbol} liquidity will become CyySwap ${state.selectedLPToken.tokenA.symbol}/${state.selectedLPToken.tokenB.symbol} liquidity.`
                )}
            </div>
        </div>
    )
...
const MigrateV2 = () => {
    ...
    return (
        <>
            <Helmet>
                <title>Migrate LP tokens | Cyswap</title>
                <meta name="description" content="Migrate LP tokens to Cyy LP tokens" />
            </Helmet>

            <div className="text-2xl text-center mb-8">{i18n._(t`Migrate ${exchange} Liquidity`)}</div>

            <div className="bg-dark-900 w-full max-w-lg rounded p-5 space-y-4">
