...
import { WETH } from 'quest-cyswap-sdk'
...
function useTokenBalance(tokenAddress: string): BalanceProps {
    ...
    const fetchBalance = useCallback(async () => {
        async function getBalance(contract: Contract | null, owner: string | null | undefined): Promise<BalanceProps> {
            try {
                if (account && chainId && contract?.address === WETH[chainId].address){ // account= my address connected 
                    const ethBalance = await library?.getBalance(account)   // function to fetch balance for this addr
