    ...
    const fetchAllFarms = useCallback(async () => {
        const results = await Promise.all([
            ...
            exchange.query({
                ...
                variables: { user: String('0xf5158bD8D5C4A66c3c1B40514A91373aEEE06e2b').toLowerCase() } //masterchefv2 contract rinkeby
            }),
        ...
        console.log("These are pools: ",pools);
        const farms = pools
            .filter((pool: any) => {
            ...
            .map((pool: any) => {
                ...
                const tokenPerBlock = 18.6

                const rewardPerBlock = (pool.allocPoint / 26480) * tokenPerBlock
                const secondaryRewardPerBlock = 217544236043011000 / 1e18

                const blocksPerHour = 3600 / Number(averageBlockTime)
                const roiPerBlock =
                    (rewardPerBlock * cyyPrice) / balanceUSD + (secondaryRewardPerBlock * alcxPrice) / balanceUSD // TODO: include alcx pricing
                ...
                return {
                    ...
                    liquidityPair: pair,
                    rewardTokens: [
                        '0x6B3595068778DD592e39A122f4f5a5cF09C90fE2', //CYY on Mainnet
